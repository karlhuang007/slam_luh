...
#            on_cylinder = True
#            sum_ray, sum_depth, rays = 0.0, 0.0, 0            
#        elif (scan_derivative > jump and on_cylinder):
...
#    return cylinder_list
        ...
        if scan_derivative[i]<-jump:# on cylinder signal
            on_cylinder = True
            sum_ray, sum_depth, rays = 0.0, 0.0, 0
        if scan[i]>min_dist and on_cylinder ==True:
            rays = rays + 1
            sum_ray = sum_ray +i
        ...
        if scan_derivative[i]>jump and on_cylinder == True:# not in cylinder signal
            if on_cylinder and rays>0:
                cylinder_list.append( (sum_ray/rays,sum_depth/rays) )
                on_cylinder = False
        #if scan_derivative[i]>jump:
...
def compute_cartesian_coordinates(cylinders, cylinder_offset):
    ...
    for c in cylinders:
        ...
        angel = LegoLogfile.beam_index_to_angle(c[0])
        x = (c[1]+cylinder_offset)*cos(angel)
        y = (c[1]+cylinder_offset)*sin(angel)
